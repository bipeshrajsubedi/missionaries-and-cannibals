...
class State(object):
	...
	def successors(self):
		...
		if self.dir == Direction.OLD_TO_NEW:
			sgn = -1
			direction = "from the left side to the right side of the river"
		else:
			sgn = 1
			direction = "from right side to the left side of the river "
		for i in self.moves:
			(m, c) = i
	...
	def addValidSuccessors(self, listChild, m, c, sgn, direction):
		...
		if newState.isValid():
			newState.action = " %d missionaries and %d cannibals %s are taken" % (m, c, direction)
